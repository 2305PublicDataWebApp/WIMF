...
후원하기 | 로그인 한 회원은 돌봄리스트에서 자신이 후원하고 싶은 유기견의 상세페이지로 넘어가서 후원하러가기 버튼을 눌러 후원페이지에서 후원을 진행할 수 있다. 후원금액은 직접입력 또는 금액버튼을 통해 입력이 가능하고
결제방식을 카카오페이 또는 신용카드로 선택해서 결제를 진행할 수 있다.
### 후기 게시판
기능명 | 상세
-------|-----
